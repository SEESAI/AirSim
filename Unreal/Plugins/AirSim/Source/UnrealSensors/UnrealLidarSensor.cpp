...
void UnrealLidarSensor::createLasers()
{
	...
	// Extract params
	float azimuth_start = params.horizontal_FOV_start;
	float azimuth_end = params.horizontal_FOV_end;
	// Conversion of "FOV upper" & "lower" to polar angles (angle away from the positive z axis)
	float polar_start = 90.0f+params.vertical_FOV_lower;
	float polar_end = 90.0f+params.vertical_FOV_upper; 
    ...
    for (int32 i = 0; i < numSectors; ++i)
    {
		...
		for (int32 j = 0; j < channels_per_scan_; ++j)
		{
			...
			if (uniform_dist_.next() < params_.point_loss_likelihood)
                ...
                int laserNb = 0; // not currently used
				if (shootLaser(lidar_pose, vehicle_pose, laserNb, azimuth_angle, polar_angle, params_, point, segmentationID)) {
