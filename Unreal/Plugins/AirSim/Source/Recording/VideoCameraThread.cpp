...
void FVideoCameraThread::stopRecording()
{
    if (instance_) {
        ...
        // while (!instance->is_complete_) // This hangs too - I think the thread is getting stuck somewhere on shutdown
    ...
    while (stop_task_counter_.GetValue() == 0)
    {
        ...
        for (const auto& vehicle_sim_api : vehicle_sim_apis_) {
            ...
            if (this_picture_taken_time >= next_screenshot_due) {
                ...
                // Set the next picture prior to its due time (to ensure it is quued correctly)
