...
public:
    ...
    virtual void update() override
    {
        ...
        const Axis3r &goal_acc_world = Axis4r::axis4ToXyz(
        ...
        msr::airlib::Vector3r body_z = msr::airlib::Vector3r(ax, ay, maxAccXy).normalized();
        ...
        collective_thrust /= (msr::airlib::Vector3r(0, 0, 1).dot(body_z));

        switch (axis_) {
        case 0: //+ay is +ve roll
        ...
        case 1: //+ax is -ve pitch
            ...
            output_ = child_controller_->getOutput();
            break;
        case 3: //+az is -ve thrust (NED coordinates)
            output_ = std::max(std::min(-collective_thrust, params_->acceleration.max_thrust), params_->acceleration.min_thrust);
            break;
        default:
            throw std::invalid_argument("axis must be 0, 1 or 3 for AccelerationController");
        }
    }
