...
struct PidConfig
{
    PidConfig(float kp_val = 0.01f, float ki_val = 0.0f, float kd_val = 0.0f,
        ...
        : kp(kp_val), ki(ki_val), kd(kd_val), time_scale(time_scale_val), min_output(min_output_val), max_output(max_output_val), enabled(enabled_val), output_bias(output_bias_val), 
