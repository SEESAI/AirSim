...
public:
    ...
    struct AngleLevelPid
    {
        ...
        //max_pitch/roll_angle > 4.0 would produce verticle thrust that is not enough to keep vehicle in air at extremities of controls
    ...
    struct VelocityPid
    {
        ...
        const float kD_XY = 0.0f; // 0.02f
        const float kI_XY = 0.02f;
        const float kP_Z = 2.0f; // 0.5f
        const float kI_Z = 2.0f; // 0.5f

        Axis4r max_limit = Axis4r(kMaxLimit, kMaxLimit, 0, kMaxLimit); //x, y, yaw, z in meters

        Axis4r p = Axis4r(kP_XY, kP_XY, 0.0f, kP_Z);
    ...
    struct Acceleration {
        const float min_thrust = 0.1f;
        const float max_thrust = 0.9f;
    } acceleration;
